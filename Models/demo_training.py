...
def fullTrain():
    ...
    #         if i > 0 and i % 4 == 0:
            ...
            if accuracy_test > 0.81:
                filepath = '../saved_model/biatae_full_train_best_83'
                print('step: %s, test loss: %s, test accuracy: %s' % (i, loss_test, accuracy_test))
                if accuracy_test > 0.83:
                    saver.save(sess, filepath)
                    break
                else:
                    if not os.path.exists(filepath):
                        saver.save(sess, '../saved_model/biatae_full_train_best_81')

        results.to_csv('../saved_model/results_%s_%s_demo.csv' % (dim_lstm, dim_aspect_embedding), index = False)
